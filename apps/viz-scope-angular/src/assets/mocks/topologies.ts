 ...
 * Migrated from viz-scope-link
 */

import { Topology, Node, Link, Variable, MetricValue } from '../../app/models';
...
export function createCoolingTopology(
  electricalNodes: ReadonlyMap<string, Node>
): Topology {
  const nodes = new Map<string, Node>();
  ...
  for (const serverId of allServerIds) {
    const server = electricalNodes.get(serverId);
    if (server) {
...
export const coolingTopology = createCoolingTopology(electricalTopology.nodes);
